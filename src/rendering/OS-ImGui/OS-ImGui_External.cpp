...
namespace OSImGui
{
	...
	std::string generateRandomString(int length)
	{
		...
		{
			randomString += std::to_string(dis(gen));
		}
	...
	void OSImGui_External::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
	{
		...
		const std::string windowName = generateRandomString(7);
		...
		std::cout << "[GUI] Creating window with name '" << windowName << "'" << std::endl;
		...
		if (DestWindow.hWnd == NULL)
			throw OSException("DestWindow does not exist");
	...
	void OSImGui_External::MainLoop()
	{
		while (!IsRunning)
		{
			...
			if (SussyAim::Cfg::Menu::StreamProof)
