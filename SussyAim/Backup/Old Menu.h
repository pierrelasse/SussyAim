...
{
	char TempText[256];
	ImGui::Begin("main", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);
	{
		ImGui::BeginTabBar("main_tabbar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip);
		...
		if (ImGui::BeginTabItem(TempText))
		{
			...
			Gui.OpenWebpageButton(TempText, "https://github.com/pierrelasse");
			ImGui::SameLine();
			sprintf_s(TempText, "%s%s", ICON_FA_COMMENT_DOTS, Lang::ReadMeText.DiscordButton);
			Gui.OpenWebpageButton(TempText, "https://discord.com/invite/3HMV4m4ga5");
			ImGui::NewLine();
